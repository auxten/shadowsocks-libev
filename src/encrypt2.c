...
{
	return ss_compress(buf_size, plaintext, len, method > TABLE ? (void *)1 : NULL);
}

char* ss_encrypt(int buf_size, char *plaintext, ssize_t *len, struct enc_ctx *ctx)
{
	return ss_compress(buf_size, plaintext, len, ctx);
}

char* ss_decrypt_all(int buf_size, char *ciphertext, ssize_t *len, int method)
{
	return ss_decompress(buf_size, ciphertext, len, method > TABLE ? (void *)1 : NULL);
}

char* ss_decrypt(int buf_size, char *ciphertext, ssize_t *len, struct enc_ctx *ctx)
{
	return ss_decompress(buf_size, ciphertext, len, ctx);
}
